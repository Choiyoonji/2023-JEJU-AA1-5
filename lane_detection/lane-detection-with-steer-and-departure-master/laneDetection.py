...
CWD_PATH = os.getcwd() + '/lane-detection-with-steer-and-departure'
...
def readVideo():
    ...
    inpImage = cv2.VideoCapture(os.getcwd() + "/lane_detection/origin_code(C version)/sample_data/track-s.mkv")
    fourcc = cv2.VideoWriter_fourcc(*'X264')
    out = cv2.VideoWriter('output.avi', fourcc, 20.0, (640,480))
    # inpImage = cv2.VideoCapture(os.path.join(CWD_PATH, 'test.mp4'))
    # inpImage = cv2.resize(inpImage, dsize=(1280, 720), dst='example', interpolation=cv2.INTER_LINEAR)
    
    # inpImage.set(cv2.CAP_PROP_FRAME_WIDTH, 640) # 가로
    # inpImage.set(cv2.CAP_PROP_FRAME_HEIGHT, 360) # 세로
    # print(inpImage.get(cv2.CAP_PROP_FRAME_WIDTH))
    # print(inpImage.get(cv2.CAP_PROP_FRAME_HEIGHT))
    # cv2.imshow("dfdf", inpImage)
    return inpImage, out
...
def perspectiveWarp(inpImage):
    ...
    # src = np.float32([[w // 2 - 30, h * 0.53], [w // 2 + 60, h * 0.53], [w * 0.3, h], [w, h]])
    # dst = np.float32([[0, 0], [w-350, 0], [400, h], [w-150, h]])
...
while True:
    ...
    # print(frame.shape)
    time.sleep(2)
    # _, frame = cv2.imread('example', image)
