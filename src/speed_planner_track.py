...
class speed_planner():
    def __init__(self):
        self.Kp = 2.0           # Speed PID control에서 Proportional 계수
    ...
    def det_target_speed(self, max_spd):
        ...
        if MODE == 1:        
            ...
            if self.max_curvature <= 20 * pi / 180:
                target_speed = MAX__SPEED
            elif self.max_curvature >= 60 * pi / 180:
                target_speed = MIN__SPEED
            else:
                target_speed = int(((MIN__SPEED - MAX__SPEED)/(60*pi/180 - 20*pi/180)) * (self.max_curvature - 20*pi/180) + MAX__SPEED)
    ...
    def speed_planner(self):
        ...
        # 급정지 상황
            ...
            brake = 80 # 동적 장애물 코스에서 속도가 50이라서
            self.brake_flag = 0
