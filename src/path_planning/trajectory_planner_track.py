...
class TrajectoryPlanner: # path planner
    ...
    def __init__(self, glob_path):
        ...
        self.S_MARGIN = 4    # 생성한 경로 끝 추가로 경로 따라서 생성할 길이
    ...
    def max_curvature_pub(self, selected_path, path_len, heading):
    ...
    def generate_path(self, si, qi, dtheta, ds = 5, qf = ROAD_WIDTH/2, path_num = 7): 
        ...
        for qf_ in np.linspace(qf, -qf, path_num): # 양수부터 차례대로 생성
            ...
            fp.s = [s for s in np.arange(si, sf, sl_d)]
    ...
    def optimal_trajectory(self, x, y, heading, obs_xy, qf = ROAD_WIDTH, path_num = 5, path_len = 4):
        ...
        if path_num == 1:
            if self.visual == True:
                ...
                self.max_curvature_pub(safe_candidate_paths[0], path_len, heading)
        ...
        if self.visual == True:
            ...
            self.max_curvature_pub(selected_path, path_len, heading)
