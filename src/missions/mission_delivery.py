...
DP = GlobalPath(gv_name)
DP2 = GlobalPath(gv_name)
'''
배달 구역 진입

1. 경로를 따라 주행하며 A 표지판 탐지(카메라팀에서 학습시켜야함)
2. 인식한 A표지판의 숫자(A1, A2, A3)를 저장
3. 표지판 위치에 가서 5초 정지
4. B 표지판 무시하고 해당 블록 주행(경로 따로 만들어서 넣자)
...
class mission_delivery:
    ...
    def __init__(self):
        ...
        self.give = False
        self.speed_ok = False
        self.info_A = [0, 0.0]          #[A표지판, A의 S좌표]
        self.info_B = [0, 0, 0, 0.0, 0.0, 0.0]
        self.loc_B = [0.0, 0.0, 0.0]    #[B위치]
        self.order_B = [0, 0, 0]
        self.count_A = [0, 0, 0]
        self.count_B = [0, 0, 0]
        self.pre_s = 0
...
################################################################################################
    ...
    def run(self, pose, heading, traffic_sign, obs, current_s):
        ...
        if (not self.speed_ok and not self.recive):    # 아직 A표지판 인식 못함
            print("break....")
            st_del = 1
            if (self.time_rec == 0):
        ...
        elif (self.speed_ok and not self.recive):  # A 표지판 인식했고 아직 정지안함
        ...
        elif (self.recive and 0.0 in self.info_B[3:]):   #A 표지판 정지 완료했고 아직 B표지판 인식 못함
            print("searching_B....")
        ...
        elif (not self.turn and not 0.0 in self.info_B[3:]):     # B 표지판 인식 했고 아직 한바퀴 안돎
            print("turning....")
            self.find_B_loc(obs)
            self.check_B_sign(traffic_sign)
            # 한바퀴 경로 넣고 따라서 크루즈
        ...
        elif (self.turn and self.speed_ok and not self.give):    # B 표지판 인식했고 아직 정지안함
        ...
        elif (self.give):   # B 표지판 정지 완료 했음 미션 끝
            print("배달 미션 끝!!!")
            st_del = 2
            # 다시 전역경로로 복귀
